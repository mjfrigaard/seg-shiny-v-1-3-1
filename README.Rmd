---
title: "SEG Shiny Heatmap (version 1.3.1)"
...
This page outlines the code used to develop the SEG application. The working version is on the Diabetes Technology Society website [here](https://www.diabetestechnology.org/seg/). The preview version (not necessarily stable) is available [here](https://n3wsandnumb3rs.shinyapps.io/seg-shiny-v-1-3-1/).

For questions, issues, or feature requests, please email Martin at martin@newsandnumbers.org.
...
1. In the 5-category summary table, there is a column labelled `REF Range`. It should be `Risk Factor Range`.

2. The heatmap-style background needs to be smoothed. Josh Senyak (josh.senyak@quicksilverconsulting.com) has provided an image for the background.  

3. The order of the entries in the Summary Table should be changed to have the order below:
...
- This contains the data for the application. Most of these data files are downloaded from the `SEG_shiny` repository [here](https://github.com/mjfrigaard/SEG_shiny/). This was changed in version 1.3.1 to [this repository](https://github.com/mjfrigaard/seg-shiny-data) because the name is more accurate. 
...
```{r BackgroundSmooth-upload, eval=FALSE}
# 1.0 - DEFINE heatmap inputs ============= ----
# 1.0.1 read BackgroundSmooth.png with readPNG ----- -----
library(jpeg)
library(png)
download.file(url = paste0(github_image_root, "BackgroundSmooth.png"),
              destfile = "www/BackgroundSmooth.png")
...
This plot requires the following data inputs (*these are also in the `helpers.R` files).

```{r gaussian-plot-helpers-inputs}
# 1.0 - DEFINE heatmap inputs ============= 
# 1.0.1 read BackgroundSmooth.png with readPNG ----- -----
library(jpeg)
library(png)
download.file(url = paste0(github_image_root, "BackgroundSmooth.png"),
              destfile = "Image/BackgroundSmooth.png")
...
This portion adds the `BackgroundSmooth` image and defines the `width` and `height` for the image.  

```{r background_smooth_layer}
# 6.2 - the background_smooth_layer ---- 
background_smooth_layer <- base_layer +
...
This layer defines the point `shape`, `fill`, `size`, `stroke` for the plot. This might be a little early, because the uploaded data is added in the final layer.

```{r points_layer_1-REMOVED}
points_layer_1 <- background_smooth_layer +
# add the points layer (1) REMOVE ----  
...
This is identical to the layer above, so `points_layer_1` can be removed.

```{r remove-points_layer_1}
rm(points_layer_1)
```

The next layer adds the red/green colors for the `color_gradient` (now in blue), the levels (`"none"`, `"slight"`, `"moderate"`, `"high"`, `"extreme"`), the barheight (`barheight = unit(100, "mm")`, and the title (`"risk level"`).

```{r scale_fill_gradientn_layer_1}
...
Next we color the `scale_color_gradientn` again, but this appears to be an unnecessary layer.  

```{r scale_fill_gradientn_layer_2-REMOVED}
...
The `scale_fill_gradientn_layer_2` layer doesn't change the plot at all, so I will remove this layer and stick with `scale_fill_gradientn_layer_1`. 

```{r remove-scale_fill_gradientn_layer_2}
rm(scale_fill_gradientn_layer_2)
```


In the next step, I format the y-axis. 

```{r y_axis_scale}
y_axis_scale <- scale_fill_gradientn_layer_1 +
...
Now that there are labels on both ends of the y-axis, I can add the labels for the x-axis below. 

```{r x_axis_scale}
x_axis_scale <- y_axis_scale +
  # 6.5 - format the x axis ---- 
  scale_x_continuous(
    limits = c(0, 600),
    sec.axis =
      sec_axis(~. / mmolConvFactor,
...
Now that we have the a nice base layer plot, we can introduce the uploaded data frame (`SampMeasData` in this case) to plot in the final layer.

```{r render-heat_map_2.0}
# 6.6 - introduce sample data frame -----
heat_map_2.0 <- x_axis_scale + 
  geom_point(
    data = SampMeasData, 
    aes(
      x = REF,
...
After reducing the layers and adding the image, the new code for the SEG graph is below. I will remove the previous layers in the code chunk below before building. 

```{r remove-layers-seg-graph-1-3-1}
rm(base_layer, background_smooth_layer, scale_fill_gradientn_layer_1, 
   y_axis_scale, x_axis_scale, heat_map_2.0)
```

In version 1.3.1, the SEG graph will be built outside of the application and added as an `.rds` object (like an uploaded data file), then the sample data will be added into it. 

```{r seg_gaussian_layer_shiny-1-3-1}
# 6.1 - create base_layer plot ---- 
base_layer <- ggplot() +
  geom_point(
    data = base_data, # defines data frame
...
Now export this object and reload the gaussian layer to plot sample data onto. 

```{r export-import-seg_gaussian_layer_shiny-as-rds, message=FALSE, warning=FALSE}
readr::write_rds(x = seg_gaussian_layer_shiny, 
                 path = "Data/seg_gaussian_layer_shiny.rds",
                 compress = "gz")
# This file has been uploaded to 
# ~/seg-shiny-data/tree/master/Data/seg_gaussian_layer_shiny.rds
...
This is the version that goes into the Shiny application in version 1.3.1
